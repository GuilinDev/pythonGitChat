...
req = response.read()
# 将数据写入文件
with open(cur_path + r"/btc_close_2017_urllib.json", 'wb') as f:  # 3
    f.write(req)
# 加载json格式
...
# 将数据写入文件
with open(cur_path + r"/btc_close_2017_request.json", 'w') as f:
    f.write(req.text)  # 2
...
# 将数据加载到一个列表中
filename = cur_path + r"/btc_close_2017.json"
...
with open(cur_path + r'收盘价Dashboard.html', 'w', encoding='utf8') as html_file:
    html_file.write(
